...
/* Esta función copia del chardev al buffer o al revés según 'dir'. 
...
sint_32 chardev_file_mov(chardev* this, void* buf, uint_32 size, char dir) {
  ...
  if (this_file->cursor < 7*FAT12_SECT_SIZE) { 
    ...
    if (i == size) return size; //Terminé de leer
  }
  ...
  while (this_file->cursor < (7+DEV_FILE_FIRST_LVL_PAGES*FAT12_SECT_PER_PAGE)*FAT12_SECT_SIZE) {
    ...
    for (i = 0; i < size && (this_file->cursor % (FAT12_SECT_SIZE * FAT12_SECT_PER_PAGE)) \
        != (FAT12_SECT_SIZE * FAT12_SECT_PER_PAGE)-1; i++) {
      dst[*idst] = src[*isrc];
      this_file->cursor++;
    }
...
chardev* fat12_open(fat12* this, const char* filename, uint_32 flags) {
  ...
  sector = this->boot_sect.ReservedSectors + (this->boot_sect.FATcount * this->boot_sect.SectorsPerFAT) \
           + ((this->boot_sect.MaxRootEntries * 32) / FAT12_SECT_SIZE) + (file_entry.FstClusLO - 2);
  size = file->size;
  //Primeros 7
...
void fat12_create(fat12* this, blockdev* dev) {
  ...
  if ((st = dev->read(dev, this->boot_sect.ReservedSectors + this->boot_sect.FATcount * this->boot_sect.SectorsPerFAT, \
          this->cache->root_dir, sectPerRoot * 512)) < 0) {
   printf("! >fat12_create: Error reading root directory.");
   return;
  }
  for (st = 0; st < FAT12_CACHE_SECTORS; st++) this->cache->sector[st] = 0; //Asigno cache como vacío
  for (st = 0; st < sectPerRoot; st++) this->cache->sector[st] = 1; //Asigno caché que tiene el root
...
int fat12_find_file(fat12* fat, const char* filename, dir_entry* entry) {
  ...
  for (i = 0; i < fat->boot_sect.MaxRootEntries; i++) {
    if (!isFile(dir[i])) continue; //No es archivo, sigo
    // for (j = 0; j < 8; j++)
      // name[j]= dir[i].Filename[j];
    // for (j = 0; j < 3; j++)
      // name[9+j] = dir[i].Extension[j];
    // printf(" >fat12_find_dir: Es archivo %s", name);
    if (!strncmp(filename+1, dir[i].Filename, min(8, strchr(filename, '.')-1)) \
        && !strncmp(filename+strchr(filename,'.')+1, dir[i].Extension, 3)) {
      // ) {
