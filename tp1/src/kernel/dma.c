...
// set DMA channel 2 to transfer data from 0x1000 - 0x33ff in memory
  ...
  // transfer length = counter + 1
      ...
      outb(0x04,0x40);         // address to 0 (low byte)
      outb(0x04,0xC0);      // address to 0x10 (high byte)
      outb(0x0c,0xFF);      // reset the master flip-flop (again!!!)
      outb(0x05,511 & 0xF);      // count to 0x23ff (low byte)
      outb(0x05,511 >> 8);      // count to 0x23ff (high byte),
      outb(0x81,0x2);        // external page register to 0 for total address of 00 10 00
...
int dma_setup() {
  ...
  // dma_set_cmd_reg(1, 0, 1); //NOTE: Utilizo lo del bios?
...
void dma_set_floppy_read(void* dest) {
  ...
  dma_set_mode(2, 1, 0, 0, 1); //Ch2, write to mem, single mode, increase, no auto-init
  outb(DMAIO_FLIPFLOP, 0xFF); //Reseteo el flip flop
  outb(DMAIO_ADDR, (uint_8)((uint_32)dest & 0xFF)); //Escribo el primer bit de la dirección
  outb(DMAIO_ADDR, (uint_8)((uint_32)dest >> 8)); //Escribo el segundo bit de la dirección
  outb(DMAIO_FLIPFLOP, 0xFF); //Reseteo el flip flop nuevamente
  outb(DMAIO_COUNT, 511 & 0xFF); //Escribo el primer byte de conteo
  outb(DMAIO_COUNT, 511 >> 8); //Escribo el segundo byte de conteo
  outb(DMAIO_TOP, ((uint_32)dest >> 16) & 0xFF); //Escribo el registro de página con los últimos 4 bits
  dma_set_mask(DMA_MASK_CH2);
}

void dma_set_floppy_write(void* src) {
  dma_set_mask(DMA_MASK_ALL);
  dma_set_mode(2, 2, 0, 0, 1); //Ch2, read from mem, single mode, increase, no auto-init
  outb(DMAIO_FLIPFLOP, 0xFF); //Reseteo el flip flop
  outb(DMAIO_ADDR, (uint_32)src & 0xFF); //Escribo el primer bit de la dirección
  outb(DMAIO_ADDR, ((uint_32)src >> 8)); //Escribo el segundo bit de la dirección
  outb(DMAIO_TOP, ((uint_32)src >> 16) & 0xFF); //Escribo el registro de página con los últimos 4 bits
  outb(DMAIO_FLIPFLOP, 0xFF); //Reseteo el flip flop nuevamente
  outb(DMAIO_COUNT, 511 & 0xFF); //Escribo el primer byte de conteo
  outb(DMAIO_COUNT, 511 >> 8); //Escribo el segundo byte de conteo
  dma_set_mask(DMA_MASK_CH2);
